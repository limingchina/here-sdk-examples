...
import com.here.evroutingkotlin.ui.theme.EVRoutingTheme
...
class MainActivity : ComponentActivity() {
    ...
    private var evRoutingExample: EVRoutingExample? = null
    ...
    fun ButtonRows() {
        Column(
            ...
            Row(
                ...
                CustomButton(onClick = { evRoutingExample?.addEVRouteButtonClicked() },
                    text = "EV Route"
                )
                Spacer(modifier = Modifier.width(16.dp))
                CustomButton(onClick = { evRoutingExample?.onReachableAreaButtonClicked() },
                    text = "Isoline"
                )
                Spacer(modifier = Modifier.width(16.dp))
                CustomButton(onClick = { evRoutingExample?.clearMap() },
                    text = "Clear"
                )
            }
        }
    ...
    private fun loadMapScene() {
        ...
        mapViewNonNull.mapScene.loadScene(MapScheme.NORMAL_DAY) { mapError ->
            if (mapError == null) {
                ...
                evRoutingExample = EVRoutingExample(this@MainActivity, mapViewNonNull)
            } else {
