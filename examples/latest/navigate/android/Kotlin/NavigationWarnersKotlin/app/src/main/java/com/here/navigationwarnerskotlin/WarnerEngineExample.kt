...
class WarnerEngineExample {
    ...
    fun setupWarnerEngine(visualNavigator: VisualNavigator) {
        ...
        warnerEngine.addWarningListener(WarningListener { warnings, warningsRegistry ->
    ...
    private fun handleWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        when (warningUpdate.warning.warningType) {
            WarningType.SAFETY_CAMERA -> handleSafetyCameraWarning(warningUpdate, warningsRegistry)
            WarningType.TRUCK_RESTRICTION -> handleTruckRestrictionWarning(warningUpdate, warningsRegistry)
            WarningType.ROAD_SIGN -> handleRoadSignWarning(warningUpdate, warningsRegistry)
            WarningType.SCHOOL_ZONE -> handleSchoolZoneWarning(warningUpdate, warningsRegistry)
            WarningType.BORDER_CROSSING -> handleBorderCrossingWarning(warningUpdate, warningsRegistry)
            WarningType.DANGER_ZONE -> handleDangerZoneWarning(warningUpdate, warningsRegistry)
            WarningType.LOW_SPEED_ZONE -> handleLowSpeedZoneWarning(warningUpdate, warningsRegistry)
            WarningType.REALISTIC_VIEW -> handleRealisticViewWarning(warningUpdate, warningsRegistry)
            WarningType.TOLL_STOP -> handleTollStopWarning(warningUpdate, warningsRegistry)
            WarningType.TRAFFIC_MERGE -> handleTrafficMergeWarning(warningUpdate, warningsRegistry)
            WarningType.CUSTOM -> handleCustomWarning(warningUpdate, warningsRegistry)
            else -> Log.d(TAG, "Unhandled warning type: ${warningUpdate.warning.warningType.name}, warning status: ${warningUpdate.warningStatus.name}")
        }
    }

    // Handles custom speed bumps warnings provided by the SpeedBumpWarningProvider.
    private fun handleCustomWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val customWarning = warningsRegistry.getCustomWarning(warningUpdate.warning)
        ...
        if (customWarning.customWarningType == SpeedBumpWarningProvider.SPEED_BUMP_WARNING_ID) {
            ...
            Log.d(TAG, "Speed bump " + (if (warningUpdate.warningStatus == WarningStatus.AHEAD) "ahead" else "passed")
                    + ". segmentRef=" + segRef)
        } else {
    ...
    private fun handleSafetyCameraWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val safetyCameraWarning = warningsRegistry.getSafetyCameraWarning(warningUpdate.warning) ?: run {
            Log.d(TAG, "SafetyCameraWarning: No detailed data available.")
            return
        }

        when (warningUpdate.warningStatus) {
            WarningStatus.AHEAD -> Log.d(TAG,
            ...
            WarningStatus.PASSED -> Log.d(TAG,
                "SafetyCameraWarning ${safetyCameraWarning.type.name} passed.")
            WarningStatus.REACHED -> Log.d(TAG,
    ...
    private fun handleTruckRestrictionWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val truckRestrictionWarning = warningsRegistry.getTruckRestrictionWarning(warningUpdate.warning) ?: run {
        ...
        when (warningUpdate.warningStatus) {
            ...
            WarningStatus.REACHED -> Log.d(TAG, "A truck restriction has been reached.")
            WarningStatus.PASSED -> Log.d(TAG, "A truck restriction just passed.")
            else -> {}
        }
    ...
    private fun handleRoadSignWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val roadSignWarning = warningsRegistry.getRoadSignWarning(warningUpdate.warning) ?: run {
    ...
    private fun handleSchoolZoneWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val schoolZoneWarning = warningsRegistry.getSchoolZoneWarning(warningUpdate.warning) ?: run {
        ...
        when (warningUpdate.warningStatus) {
            ...
            WarningStatus.REACHED -> Log.d(TAG, "A school zone has been reached.")
            WarningStatus.PASSED -> Log.d(TAG, "A school zone has been passed.")
            else -> {}
        }
    }
    ...
    private fun handleBorderCrossingWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val borderCrossingWarning = warningsRegistry.getBorderCrossingWarning(warningUpdate.warning) ?: run {
        ...
        when (warningUpdate.warningStatus) {
            ...
            WarningStatus.PASSED -> Log.d(TAG, "A border crossing has been passed.")
            else -> {}
        }
    }
    ...
    private fun handleDangerZoneWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val dangerZoneWarning = warningsRegistry.getDangerZoneWarning(warningUpdate.warning) ?: run {
        ...
        when (warningUpdate.warningStatus) {
            ...
            WarningStatus.REACHED -> Log.d(TAG, "A danger zone has been reached. isZoneStart: ${dangerZoneWarning.isZoneStart}")
            WarningStatus.PASSED -> Log.d(TAG, "A danger zone has been passed.")
            else -> {}
        }
    }
    ...
    private fun handleLowSpeedZoneWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val lowSpeedZoneWarning = warningsRegistry.getLowSpeedZoneWarning(warningUpdate.warning) ?: run {
        ...
        when (warningUpdate.warningStatus) {
            ...
            WarningStatus.REACHED -> {
                Log.d(TAG, "A low speed zone has been reached.")
                Log.d(TAG, "Speed limit in low speed zone (m/s): ${lowSpeedZoneWarning.speedLimitInMetersPerSecond}")
            }
            WarningStatus.PASSED -> Log.d(TAG, "A low speed zone has been passed.")
            else -> {}
        }
    }
    ...
    private fun handleRealisticViewWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val realisticViewWarning = warningsRegistry.getRealisticViewWarning(warningUpdate.warning) ?: run {
        ...
        when (warningUpdate.warningStatus) {
            WarningStatus.AHEAD -> Log.d(TAG, "RealisticView ahead in: $distance meters.")
            WarningStatus.PASSED -> Log.d(TAG, "A RealisticView just passed.")
            else -> {}
        }
    ...
    private fun handleTollStopWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val tollStop = warningsRegistry.getTollStopWarning(warningUpdate.warning) ?: run {
    ...
    private fun handleTrafficMergeWarning(warningUpdate: WarningUpdate, warningsRegistry: WarningsRegistry) {
        val trafficMergeWarning = warningsRegistry.getTrafficMergeWarning(warningUpdate.warning) ?: run {
